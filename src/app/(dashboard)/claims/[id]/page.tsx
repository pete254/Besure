...
import { ArrowLeft, Car, User, MapPin, AlertTriangle, MessageSquare, ChevronRight, Check } from "lucide-react";
...
interface Claim {
  ...
  thirdPartyDetails?: any; repairEstimate?: string | null;
  approvedAmount?: string | null; settlementDate?: string | null;
  settlementMethod?: string | null; garageFreeText?: string | null;
}
...
const STAGES = ["Reported", "Documents Pending", "Fully Documented", "Assessed", "Executed", "Approved", "Released / Settled"];
...
export default function ClaimDetailPage() {
  ...
  const [financials, setFinancials] = useState({ repairEstimate: "", approvedAmount: "", settlementDate: "", settlementMethod: "" });
  ...
  const currentStageIndex = STAGES.indexOf(claim.stage);
  ...
  return (
    <div style={{ maxWidth: "960px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "16px" }}>
      <Link href="/claims" style={{ display: "inline-flex", alignItems: "center", gap: "6px", textDecoration: "none", fontSize: "13px", color: "var(--text-muted)" }}
      ...
      <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
              ...
              <span style={{ padding: "3px 10px", borderRadius: "20px", fontSize: "11px", fontWeight: 600, backgroundColor: stageStyle.bg, color: stageStyle.color }}>{claim.stage}</span>
            </div>
        ...
        {customer && (
          <Link href={`/customers/${customer.id}`} style={{ display: "flex", alignItems: "center", gap: "6px", padding: "7px 12px", borderRadius: "8px", border: "1px solid var(--border)", textDecoration: "none", fontSize: "13px", fontWeight: 600, color: "var(--text-secondary)", transition: "border-color 0.1s" }}
      ...
      <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
        <p style={{ fontSize: "13px", fontWeight: 700, color: "var(--text-primary)", marginBottom: "16px" }}>Claim Stage</p>
              ...
              <div key={stage} style={{ display: "flex", alignItems: "center", gap: "4px", flexShrink: 0 }}>
                <button
                  ...
                  style={{
                    ...
                    borderRadius: "20px", border: `1px solid ${active ? colors.color : done ? "var(--border)" : "transparent"}`,
                ...
                >
                  {done && <Check size={10} color="var(--brand)" />}
                  {stage}
                </button>
                {i < STAGES.length - 1 && <ChevronRight size={12} color="var(--text-muted)" />}
              </div>
            );
          })}
        </div>
            ...
            {showFinancials ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                {[
                  ...
                  <div key={key}>
                    <label style={{ display: "block", fontSize: "11px", fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "4px" }}>{label}</label>
                    <input type={type} value={(financials as any)[key]} onChange={(e) => setFinancials(prev => ({ ...prev, [key]: e.target.value }))}
      ...
      <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
        ...
        <div style={{ display: "flex", gap: "10px", marginBottom: "16px" }}>
          <select value={noteChannel} onChange={(e) => setNoteChannel(e.target.value)}
          ...
          <input value={noteText} onChange={(e) => setNoteText(e.target.value)} placeholder="Add a follow-up note..."
