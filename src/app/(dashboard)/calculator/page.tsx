...
// Standalone premium calculator — quick quotes without saving a policy

"use client";

import { useState, useEffect } from "react";
import { Calculator, RefreshCw, Copy, Check, ChevronDown, ChevronUp, Plus, Trash2 } from "lucide-react";
...
export default function CalculatorPage() {
  ...
  // Live re-calculate when inputs change
  ...
  function buildQuoteSummary() {
    ...
    const lines = [
      ...
      ``,
      `Statutory Levies:`,
      `  IRA Levy (0.45%):    ${fmt(result.iraLevy)}`,
      `  Training Levy (0.2%):${fmt(result.trainingLevy)}`,
    ...
    <div style={{ maxWidth: "900px", margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
        ...
        <button
          ...
          onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.color = "#ffffff"; (e.currentTarget as HTMLElement).style.borderColor = "var(--text-muted)"; }}
              ...
              {INSURANCE_TYPES.map((t) => (
                <label
                  ...
                  style={{
                    ...
                    border: `1px solid ${insuranceType === t.value ? "var(--brand)" : "var(--border)"}`,
                    backgroundColor: insuranceType === t.value ? "rgba(16,185,129,0.08)" : "var(--bg-app)",
                  }}
                >
                  <input type="radio" name="insuranceType" value={t.value}
                    checked={insuranceType === t.value}
                    onChange={(e) => setInsuranceType(e.target.value)}
                    style={{ width: "auto", margin: 0, accentColor: "var(--brand)" }}
                  />
                  <span style={{ fontSize: "13px", fontWeight: 600, color: insuranceType === t.value ? "var(--brand)" : "var(--text-secondary)" }}>
                    {t.label}
                  </span>
                </label>
              ))}
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "18px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <div>
                ...
                <select value={selectedInsurerId} onChange={(e) => {
                  if (e.target.value === "manual") setSelectedInsurerId("");
                  else setSelectedInsurerId(e.target.value);
              ...
              <div>
                ...
                {selectedInsurerId && (
                  <p style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "3px" }}>
                    Auto-filled from insurer — editable
                  </p>
                )}
              </div>
            </div>
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", overflow: "hidden" }}>
            <button
              ...
              style={{
                ...
                borderBottom: showBenefits ? "1px solid var(--border)" : "none",
              }}
            >
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        ...
                        {selected && (
                          <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                            ...
                            <input
                              type="number"
                              value={selected.amountKes}
                              onChange={(e) => updateBenefitAmount(b.id, e.target.value)}
                              placeholder="0.00"
          ...
          {!result && !loading && (
            <div style={{
              ...
              <p style={{ fontSize: "13px", color: "var(--text-muted)" }}>
                Fill in the sum insured and rate on the left — the premium will calculate automatically.
              </p>
            </div>
          )}
            ...
            <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", overflow: "hidden" }}>
              ...
              <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  ...
                  {result.insurer && (
                    <p style={{ fontSize: "12px", color: "var(--text-muted)", margin: "2px 0 0" }}>{result.insurer.name} · {insuranceType.replace("Motor - ", "")}</p>
                  )}
                </div>
                <button
                  onClick={() => copyToClipboard(buildQuoteSummary(), setCopied)}
              ...
              <div style={{ padding: "16px 18px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid var(--border)" }}>
                  <div>
                    ...
                    {result.minimumApplied && (
                      <span style={{ display: "block", fontSize: "10px", color: "#fbbf24", marginTop: "1px" }}>
                        ⚠ Minimum premium applied ({fmt(result.minPremium || 0)})
                      </span>
                    )}
                  </div>
                ...
                {[
                  { label: `IRA Levy (0.45%)`, value: result.iraLevy },
                  { label: `Training Levy (0.2%)`, value: result.trainingLevy },
                  { label: `Stamp Duty`, value: result.stampDuty },
                  { label: `PHCF`, value: result.phcf },
                ].map(({ label, value }) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid var(--border)" }}>
                    ...
                    <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>{fmt(value)}</span>
                  </div>
                ))}
                ...
                {result.commissionRate > 0 && (
                  <div style={{ marginTop: "10px", padding: "10px 12px", backgroundColor: "rgba(167,139,250,0.08)", borderRadius: "8px", border: "1px solid rgba(167,139,250,0.2)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: "12px", color: "#a78bfa", fontWeight: 600 }}>
                        Agency Commission ({result.commissionRate}%)
                      </span>
                      <span style={{ fontSize: "12px", fontWeight: 700, color: "#a78bfa" }}>{fmt(result.agencyCommission)}</span>
                    </div>
                    <p style={{ fontSize: "10px", color: "var(--text-muted)", margin: "3px 0 0" }}>Internal reference — not charged to client</p>
                  </div>
                )}

                {/* Installment scenarios */}
                <div style={{ marginTop: "14px" }}>
                  <p style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-muted)", marginBottom: "8px" }}>
                    Installment Scenarios
                  </p>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px" }}>
                    {[
                      ...
                      <div key={label} style={{ padding: "8px 10px", backgroundColor: "var(--bg-app)", borderRadius: "6px", border: "1px solid var(--border)", textAlign: "center" }}>
                        ...
                        <p style={{ fontSize: "12px", fontWeight: 700, color: "var(--text-primary)", margin: "0 0 1px" }}>
                          {fmt(amount)}
                        </p>
              ...
              <div style={{ padding: "12px 18px", borderTop: "1px solid var(--border)", display: "flex", gap: "8px" }}>
                <a
                  ...
                  style={{
                    ...
                    padding: "9px", backgroundColor: "var(--brand)", color: "#000", border: "none",
