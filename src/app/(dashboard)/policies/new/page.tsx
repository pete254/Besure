...
interface BenefitEntry {
  ...
  benefitName: string;
  amountKes: string;
  // Extra fields for specific benefits
...
interface PolicyData {
  ...
  vehicle: { make: string; model: string; year: string; cc: string; tonnage: string; seats: string; chassisNo: string; engineNo: string; regNo: string; bodyType: string; colour: string; };
  coverType: string; sumInsured: string; startDate: string; endDate: string; policyNumber: string;
  basicRate: string; basicPremium: string; iraLevy: string; trainingLevy: string; stampDuty: string; phcf: string;
...
const STEPS = ["Insurance Type", "Customer & Insurer", "Vehicle", "Cover & Valuation", "Rate & Premium", "Benefits", "Summary & Payment"];

// Benefits that have special auto-calculation
...
const emptyPolicy: PolicyData = {
  ...
  basicRate: "", basicPremium: "", iraLevy: "", trainingLevy: "", stampDuty: "40", phcf: "100",
...
export default function NewPolicyPage() {
  ...
  const [data, setData] = useState<PolicyData>({ ...emptyPolicy, customerId: searchParams.get("customerId") || "" });
  ...
  useEffect(() => {
    fetch("/api/insurers").then(r => r.json()).then(d => setInsurers(d.insurers?.filter((i: Insurer) => i.isActive) || []));
    fetch("/api/benefits").then(r => r.json()).then(d => setBenefits(d.benefits?.filter((b: Benefit) => b.isActive) || []));
  }, []);
  ...
  useEffect(() => {
    ...
    if (sum > 0 && rate > 0) {
      ...
      const training = (parseFloat(basic) * 0.002).toFixed(2);
      setData(prev => ({ ...prev, basicPremium: basic, iraLevy: ira, trainingLevy: training }));
    }
    ...
    setData(prev => {
      const updated = prev.benefits.map(b => {
        if (b.benefitName === "Excess Protector") {
          const sum = parseFloat(prev.sumInsured || "0");
          const auto = (sum * 0.005).toFixed(2);
          return { ...b, amountKes: auto };
        }
        return b;
      });
      const total = updated.reduce((s, b) => s + parseFloat(b.amountKes || "0"), 0).toFixed(2);
      return { ...prev, benefits: updated, totalBenefits: total };
    });
  }, [data.sumInsured]);
  ...
  useEffect(() => {
    ...
    const training = parseFloat(data.trainingLevy || "0");
    const stamp = parseFloat(data.stampDuty || "40");
    const phcf = parseFloat(data.phcf || "100");
    const total = (basic + bens + ira + training + stamp + phcf).toFixed(2);
    setData(prev => ({ ...prev, grandTotal: total }));
  }, [data.basicPremium, data.totalBenefits, data.iraLevy, data.trainingLevy, data.stampDuty, data.phcf]);
  ...
  function recalcBenefitTotal(updated: BenefitEntry[]) {
    ...
    setData(prev => {
      ...
      if (exists) {
        ...
        if (bType === "excess") {
          ...
          autoAmount = "30000"; // default 10 days × 3000
        }
        updated = [...prev.benefits, { benefitOptionId: b.id, benefitName: b.name, amountKes: autoAmount }];
      }
      return { ...prev, benefits: updated, totalBenefits: recalcBenefitTotal(updated) };
    });
  }
  ...
  function updateBenefitField(id: string, field: keyof BenefitEntry, value: string) {
    setData(prev => {
      const updated = prev.benefits.map(b => {
        ...
        // Auto-recalculate windscreen amount
        ...
        // Auto-recalculate loss of use
        if (field === "lossOfUseDays" && b.benefitName === "Loss of Use / Car Hire") {
          const days = parseInt(value || "0");
          entry.amountKes = (days * 3000).toFixed(2);
        }
        return entry;
      });
      return { ...prev, benefits: updated, totalBenefits: recalcBenefitTotal(updated) };
  ...
  async function handleSubmit() {
    ...
    try {
      const payload = {
        ...data,
        vehicle: isMotor ? { ...data.vehicle, year: parseInt(data.vehicle.year), cc: data.vehicle.cc ? parseInt(data.vehicle.cc) : null, seats: data.vehicle.seats ? parseInt(data.vehicle.seats) : null } : null,
  ...
  const inStyle: React.CSSProperties = { width: "100%", padding: "9px 12px", backgroundColor: "var(--bg-app)", border: "1px solid var(--border)", borderRadius: "8px", color: "var(--text-primary)", fontSize: "13px", outline: "none" };
  const lbStyle: React.CSSProperties = { display: "block", fontSize: "12px", fontWeight: 600, color: "var(--text-secondary)", marginBottom: "5px", textTransform: "uppercase", letterSpacing: "0.05em" };
  function foc(e: React.FocusEvent<HTMLInputElement | HTMLSelectElement>) { (e.target as HTMLElement).style.borderColor = "var(--brand)"; }
  function blr(e: React.FocusEvent<HTMLInputElement | HTMLSelectElement>) { (e.target as HTMLElement).style.borderColor = "var(--border)"; }

  return (
    <div style={{ maxWidth: "760px", margin: "0 auto" }}>
      <Link href="/policies" style={{ display: "inline-flex", alignItems: "center", gap: "6px", textDecoration: "none", marginBottom: "20px", fontSize: "13px", color: "var(--text-muted)" }}
            ...
            <div key={label} style={{ display: "flex", alignItems: "center", gap: "4px", flexShrink: 0 }}>
              <div onClick={() => done && setStep(num)}
                ...
                <div style={{ width: "18px", height: "18px", borderRadius: "50%", backgroundColor: active ? "var(--brand)" : done ? "var(--bg-card)" : "var(--border)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {done ? <Check size={10} color="var(--brand)" /> : <span style={{ fontSize: "10px", fontWeight: 700, color: active ? "#000" : "var(--text-muted)" }}>{num}</span>}
                </div>
                <span style={{ fontSize: "11px", fontWeight: 600, color: active ? "var(--brand)" : done ? "var(--text-secondary)" : "var(--text-muted)", whiteSpace: "nowrap" }}>{label}</span>
              </div>
        ...
        <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
            {INSURANCE_TYPES.map((t) => (
              <label key={t.value} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 14px", borderRadius: "8px", cursor: "pointer", border: `1px solid ${data.insuranceType === t.value ? "var(--brand)" : "var(--border)"}`, backgroundColor: data.insuranceType === t.value ? "rgba(16,185,129,0.08)" : "var(--bg-app)" }}>
                <input type="radio" name="insuranceType" value={t.value} checked={data.insuranceType === t.value} onChange={(e) => setData(prev => ({ ...prev, insuranceType: e.target.value }))} style={{ width: "auto", margin: 0 }} />
                <span style={{ fontSize: "13px", fontWeight: 600, color: data.insuranceType === t.value ? "var(--brand)" : "var(--text-secondary)" }}>{t.label}</span>
                {!t.motor && <span style={{ marginLeft: "auto", padding: "1px 6px", borderRadius: "10px", fontSize: "10px", fontWeight: 600, backgroundColor: "rgba(107,114,128,0.15)", color: "#9ca3af" }}>Phase 2</span>}
              </label>
            ))}
          </div>
        </div>
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
            ...
            {data.customerId && data.customerName ? (
              ...
              <div style={{ position: "relative" }}>
                <input placeholder="Search customer by name or phone..." value={customerSearch} onChange={(e) => setCustomerSearch(e.target.value)} style={inStyle} onFocus={foc} onBlur={blr} />
                {customerResults.length > 0 && (
                  <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 10, backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "8px", marginTop: "4px", overflow: "hidden" }}>
                    {customerResults.map((c) => {
                      ...
                      return (
                        <div key={c.id} onClick={() => { setData(prev => ({ ...prev, customerId: c.id, customerName: name })); setCustomerSearch(""); setCustomerResults([]); }}
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <div>
                ...
                <select value={data.insurerId} onChange={(e) => { if (e.target.value === "manual") { setData(prev => ({ ...prev, insurerId: "" })); } else { handleInsurerSelect(e.target.value); } }} style={inStyle} onFocus={foc} onBlur={blr}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "14px" }}>
              {[
                ...
                <div key={key}>
                  ...
                  <input placeholder={placeholder} value={(data.vehicle as any)[key]}
              ...
              <div>
                ...
                <select value={data.vehicle.bodyType} onChange={(e) => setData(prev => ({ ...prev, vehicle: { ...prev.vehicle, bodyType: e.target.value } }))} style={inStyle} onFocus={foc} onBlur={blr}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  {COVER_TYPES.map((ct) => (
                    <label key={ct} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", padding: "10px", borderRadius: "8px", cursor: "pointer", border: `1px solid ${data.coverType === ct ? "var(--brand)" : "var(--border)"}`, backgroundColor: data.coverType === ct ? "rgba(16,185,129,0.08)" : "var(--bg-app)" }}>
                      <input type="radio" name="coverType" value={ct} checked={data.coverType === ct} onChange={(e) => setData(prev => ({ ...prev, coverType: e.target.value }))} style={{ width: "auto", margin: 0 }} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
              <div>
                ...
                <input type="number" placeholder="1500000" value={data.sumInsured} onChange={(e) => setData(prev => ({ ...prev, sumInsured: e.target.value }))} style={inStyle} onFocus={foc} onBlur={blr} />
              </div>
              <div>
                <label style={lbStyle}>Policy Number</label>
                <input placeholder="Issued by insurer — can add later" value={data.policyNumber} onChange={(e) => setData(prev => ({ ...prev, policyNumber: e.target.value }))} style={inStyle} onFocus={foc} onBlur={blr} />
              </div>
              <div>
                <label style={lbStyle}>Policy Start Date *</label>
                <input type="date" value={data.startDate} onChange={(e) => setData(prev => ({ ...prev, startDate: e.target.value }))} style={inStyle} onFocus={foc} onBlur={blr} />
              </div>
              <div>
                <label style={lbStyle}>Policy End Date *</label>
                <input type="date" value={data.endDate} onChange={(e) => setData(prev => ({ ...prev, endDate: e.target.value }))} style={inStyle} onFocus={foc} onBlur={blr} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}>
              <div>
                ...
                <input type="number" step="0.01" placeholder="4.00" value={data.basicRate} onChange={(e) => setData(prev => ({ ...prev, basicRate: e.target.value }))} style={inStyle} onFocus={foc} onBlur={blr} />
              ...
              <div>
                ...
                <input value={data.sumInsured ? `KES ${parseFloat(data.sumInsured).toLocaleString()}` : "—"} readOnly style={{ ...inStyle, backgroundColor: "var(--bg-sidebar)", cursor: "not-allowed", color: "var(--text-secondary)" }} />
              </div>
            </div>
            <div style={{ backgroundColor: "var(--bg-app)", borderRadius: "8px", border: "1px solid var(--border)", padding: "16px" }}>
              <p style={{ fontSize: "12px", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "12px" }}>Premium Breakdown</p>
              {[
                { label: "Basic Premium", value: data.basicPremium },
                { label: "IRA Levy (0.45%)", value: data.iraLevy },
                { label: "Training Levy (0.2%)", value: data.trainingLevy },
      ...
      {step === 6 && (
        <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
          ...
          <p style={{ fontSize: "12px", color: "var(--text-muted)", marginBottom: "14px" }}>Sum Insured: <strong style={{ color: "var(--brand)" }}>{formatKES(data.sumInsured)}</strong></p>
              ...
              return (
                <div key={b.id} style={{ padding: "14px", borderRadius: "8px", border: `1px solid ${selected ? "var(--brand)" : "var(--border)"}`, backgroundColor: selected ? "rgba(16,185,129,0.04)" : "var(--bg-app)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <input type="checkbox" checked={!!selected} onChange={() => toggleBenefit(b)} style={{ width: "16px", height: "16px", margin: 0, accentColor: "var(--brand)" }} />
                    <span style={{ fontSize: "13px", fontWeight: 600, color: selected ? "var(--text-primary)" : "var(--text-secondary)", flex: 1 }}>{b.name}</span>
                  ...
                  {/* Excess Protector — auto-calculated, editable */}
                  {selected && bType === "excess" && (
                    <div style={{ marginTop: "10px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                      <div>
                        <label style={{ fontSize: "11px", fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", display: "block", marginBottom: "4px" }}>Auto-calculated (0.5% of sum insured)</label>
                        <input type="number" value={selected.amountKes} onChange={(e) => updateBenefitField(b.id, "amountKes", e.target.value)}
                          ...
                          onFocus={(e) => { (e.target as HTMLInputElement).style.borderColor = "var(--brand-hover)"; }}
                          onBlur={(e) => { (e.target as HTMLInputElement).style.borderColor = "var(--brand)"; }} />
                        <p style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "3px" }}>Editable — auto-updates when sum insured changes</p>
                      </div>
                    </div>
                  )}

                  {/* Windscreen — needs windscreen value, auto-calculates */}
                    ...
                    <div style={{ marginTop: "10px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                      <div>
                        ...
                        <p style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "3px" }}>
                          ≤ KES 50,000 = free · Above: 10% of windscreen value
                        </p>
                      </div>
                      <div>
                        <label style={{ fontSize: "11px", fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", display: "block", marginBottom: "4px" }}>Windscreen Premium (KES)</label>
                        <input type="number" value={selected.amountKes}
                          onChange={(e) => updateBenefitField(b.id, "amountKes", e.target.value)}
                          style={{ width: "100%", padding: "7px 10px", backgroundColor: "var(--bg-card)", border: "1px solid var(--brand)", borderRadius: "6px", color: "var(--brand)", fontSize: "13px", outline: "none", fontWeight: 600 }}
                          onFocus={(e) => { (e.target as HTMLInputElement).style.borderColor = "var(--brand-hover)"; }}
                          onBlur={(e) => { (e.target as HTMLInputElement).style.borderColor = "var(--brand)"; }} />
                        <p style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "3px" }}>Auto-calculated — editable</p>
                      </div>
                    </div>
                  )}

                  {/* Loss of Use — days selector */}
                  {selected && bType === "loss_of_use" && (
                    <div style={{ marginTop: "10px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                      <div>
                        ...
                        <p style={{ fontSize: "11px", color: "var(--text-muted)", marginTop: "3px" }}>KES 3,000 per day</p>
                      </div>
                      <div>
                  ...
                  {/* Entertainment Unit — custom amount */}
      ...
      {step === 7 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
            <p style={{ fontSize: "13px", fontWeight: 700, color: "var(--text-primary)", marginBottom: "16px", paddingBottom: "10px", borderBottom: "1px solid var(--border)" }}>Premium Summary</p>
            {[
              { label: "Sum Insured", value: data.sumInsured },
              { label: "Basic Premium", value: data.basicPremium },
              { label: "Total Benefits", value: data.totalBenefits },
              { label: "IRA Levy (0.45%)", value: data.iraLevy },
              { label: "Training Levy (0.2%)", value: data.trainingLevy },
              { label: "Stamp Duty", value: data.stampDuty },
              { label: "PHCF", value: data.phcf },
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
              {PAYMENT_MODES.map((mode) => (
                <label key={mode} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 14px", borderRadius: "8px", cursor: "pointer", border: `1px solid ${data.paymentMode === mode ? "var(--brand)" : "var(--border)"}`, backgroundColor: data.paymentMode === mode ? "rgba(16,185,129,0.08)" : "var(--bg-app)" }}>
                  <input type="radio" name="paymentMode" value={mode} checked={data.paymentMode === mode} onChange={(e) => setData(prev => ({ ...prev, paymentMode: e.target.value }))} style={{ width: "auto", margin: 0 }} />
            ...
            {data.paymentMode === "IPF" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", marginTop: "14px" }}>
                <div><label style={lbStyle}>IPF Provider</label><input placeholder="e.g. Mogo Finance" value={data.ipfProvider} onChange={(e) => setData(prev => ({ ...prev, ipfProvider: e.target.value }))} style={inStyle} onFocus={foc} onBlur={blr} /></div>
                <div><label style={lbStyle}>Loan Reference</label><input placeholder="Loan ref number" value={data.ipfLoanReference} onChange={(e) => setData(prev => ({ ...prev, ipfLoanReference: e.target.value }))} style={inStyle} onFocus={foc} onBlur={blr} /></div>
              </div>
            )}
          </div>
          ...
          <div style={{ backgroundColor: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "20px" }}>
            ...
            <textarea placeholder="Any additional notes..." value={data.notes} onChange={(e) => setData(prev => ({ ...prev, notes: e.target.value }))} rows={3}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "20px" }}>
        ...
        {step < 7 ? (
          <button className="btn-primary" disabled={!canProceed()}
            onClick={() => { if (!isMotor && step === 2) { setStep(4); return; } setStep(s => s + 1); }}>
